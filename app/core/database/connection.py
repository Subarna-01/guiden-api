...
class DbConnectionManager:
    ...
    DATABASE_URL: str = f"postgresql://{settings.POSTGRES_USER}:{settings.POSTGRES_PASSWORD}@{settings.POSTGRES_HOST}:{settings.POSTGRES_PORT}"
    ...
    def init_engines(self, db_names):
        for db_name in db_names:
            engine = create_engine(f"{self.DATABASE_URL}/{db_name}", echo=False, future=True)
