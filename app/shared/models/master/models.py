from sqlalchemy import Column, BigInteger, String, Text, Boolean, TIMESTAMP, ForeignKey, Integer
...
class Country(BaseMasterDb):
    ...
    document_validations = relationship("GovernmentDocumentValidCountry", back_populates="country", cascade="all, delete-orphan")
...
class GovernmentDocumentType(BaseMasterDb):
    ...
    document_type_id = Column("document_type_id", Integer, primary_key=True, autoincrement=True)
    document_type = Column("document_type", String(50), unique=True, nullable=False)
    is_active = Column("is_active", Boolean, nullable=False, default=True)
    created_at = Column("created_at", TIMESTAMP(timezone=True), server_default=func.now())
    updated_at = Column("updated_at", TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now())

    valid_countries = relationship("GovernmentDocumentValidCountry", back_populates="document_type", cascade="all, delete-orphan")

class GovernmentDocumentValidCountry(BaseMasterDb):
    __tablename__ = "government_document_valid_countries"
    __table_args__ = {"extend_existing": True}

    document_type_id = Column("document_type_id", Integer, ForeignKey("government_document_types.document_type_id", ondelete="CASCADE"), primary_key=True, nullable=False)
    country_id = Column("country_id", BigInteger, ForeignKey("countries.country_id", ondelete="CASCADE"), nullable=False)
    is_active = Column("is_active", Boolean, nullable=False, default=True)
    created_at = Column("created_at", TIMESTAMP(timezone=True), server_default=func.now())
    updated_at = Column("updated_at", TIMESTAMP(timezone=True), server_default=func.now(), onupdate=func.now())

    document_type = relationship("GovernmentDocumentType", back_populates="valid_countries")
